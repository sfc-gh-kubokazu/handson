...
```
SNOWRETAIL_DB.SNOWRETAIL_SCHEMA の MART_SALES と CUSTOMER_REVIEWS を使って
Semantic View を作成してください。

MART_SALES は、Step 1 の setup.sql で作成済みの Dynamic Table で、
EC と実店舗の売上トランザクションを統合し、商品IDから BRAND ・ CATEGORY を導出済み。
含めるメトリクス:
- 総売上金額 (TOTAL_PRICE の合計)
- 総売上数量 (QUANTITY の合計)
- チャネル別売上金額 (CHANNEL ごとの集計)
- レビュー平均評価 (CUSTOMER_REVIEWS.RATING の平均)

含めるディメンション:
- 商品カテゴリ (MART_SALES.CATEGORY)
- ブランド (MART_SALES.BRAND)
- 販売チャネル (MART_SALES.CHANNEL: 'EC' / 'RETAIL')
- 売上日 (MART_SALES.TRANSACTION_DATE — 月単位で集計できるように)

MART_SALES と CUSTOMER_REVIEWS は PRODUCT_ID で関連付けてください。
Semantic View 名: SNOWRETAIL_SV
```
...
1. SNOWRETAIL_SV（売上・レビューの構造化データ分析）
