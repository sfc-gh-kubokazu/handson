...
### 📚 補足: Cortex Analyst とセマンティックビュー

Cortex Agent の中核ツールである **Cortex Analyst** は、自然言語の質問を SQL に変換するサービスです。

![Cortex Analyst Overview](../assets/screenshots/02_agent/01_cortex_analyst_overview.png)

#### Cortex Analyst の仕組み

![Cortex Analyst Flow](../assets/screenshots/02_agent/02_cortex_analyst_flow.png)

REST API 経由で質問 → セマンティックモデルを参照 → SQL 生成 → Snowflake で実行 → 結果返却、の流れ。

#### Snowflake セマンティックビュー

![Semantic View](../assets/screenshots/02_agent/03_semantic_view.png)

テーブルに対するビジネス用語（メトリクス・ディメンション・関係）の意味付け層。Cortex Analyst が参照することで、ユーザーの自然言語と実テーブルとの間のギャップを埋めます。
...
【tool_resources】
- Analyst の execution_environment に warehouse: WH_HANDSON を必ず指定
  （これが無いと "Analyst tool is missing an execution environment" エラーになります）
