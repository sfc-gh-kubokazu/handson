...
| 名前 | これは何か |
|---|---|
| **`AI_PARSE_DOCUMENT`** | ステージ上の文書からテキスト・表・レイアウトを取り出す Cortex AI 関数。PDF / PPTX / DOCX / 画像 / HTML / TXT に対応し、1ファイル100MB・2,000ページまで。読み取り順や見出し・表の構造を保ったまま Markdown で返す |
| **`TO_FILE`** | ステージ上のファイルを、SQLで扱える FILE オブジェクトに変換する関数。AI関数にファイルを渡すときの入口になる |
| **`DIRECTORY`** | ステージのファイル一覧をテーブルのように扱えるテーブル関数。ファイルを追加したら `ALTER STAGE ... REFRESH` が必要 |
| **`SPLIT_TEXT_RECURSIVE_CHARACTER`** | 長いテキストを指定サイズのチャンクに分割する関数。`markdown` などの書式を指定すると、見出しや表の区切りを尊重して切ってくれる |
| **`COUNT_TOKENS`** | 指定したモデルでのトークン数を数える関数。日本語は文字数とトークン数が一致しないため、チャンクサイズの検証に使う |
